...
impl RubiksCubeModelInterface {
    pub fn new(load_from_file: Option<String>) -> Self {
        let num_trajectories = 400;
        let trajectory_depth = 2;
        let num_layers: u32 = 5;
        let hidden_layer_dimension: u32 = 50; //2000;
        let num_epochs: u32 = 10;
    ...
    fn randomly_scramble_cube(
        cube: &RubiksCube,
        num_starting_points: u32,
        turns: u32,
        same_end: bool,
        ...
        for i in 0..turns {
            fixed_moves.push(
                Self::random_sample_move(), // CubeMove::FPlus
                                            // CubeMove::FMinus
                                            // CubeMove::UPlus
                                            // CubeMove::UMinus
                                            // CubeMove::DMinus
    ...
    pub fn test_logits(&self) {
        ...
        println!(
            "Got the cubemove logits from trained policy: {:?}",
            logits.size()
        );
    ...
    pub fn test_policy(&mut self) {
        ...
        let r_cubes_moves: (Vec<RubiksCube>, Vec<Vec<CubeMove>>) =
            Self::randomly_scramble_cube(&mut self.rubiks_cube, 10, self.trajectory_depth, false);
        info!(
            "The random scrambling applied these moves: {:?} for testing",
            r_cubes_moves.1
        );
        ...
        for i in 0..self.num_tests as usize {
            ...
            info!(
                "Scrambled cube used for test {:?} used this moves: {:?}",
                i, r_move
            );
            ...
            for j in 0..self.trajectory_depth {
                ...
                let mvc = r_move
                    .get((self.trajectory_depth - j - 1) as usize)
                    .expect("Expected move to be available");
                ...
                info!("The cross entropy for turn {:?} {:?}", j, policy_entropy_f);
        ...
        info!(
            "Mean loss for tests {:?}",
            mean_loss / (self.num_tests as f32)
        );
        info!(
            "Mean entropy for tests {:?}",
            mean_entropy / ((self.num_tests * self.trajectory_depth) as f32)
        );
        info!(
            "Mean correct moves for {:?}",
            num_correct_moves as f32 / self.num_tests as f32
        );
    }
    ...
    pub fn train_policy(&mut self) {
        ...
        for i in 0..self.solver.num_epochs {
            ...
            let r_cubes_moves = Self::randomly_scramble_cube(
            ...
            println!(
                "The random scrambling applied these moves: {:?} for training epoch {:?}",
                r_cubes_moves.1, i
            );

            let trajs: Vec<Trajectory> = self.solver.gen_trajectories(r_cubes_moves.0.clone());
            // println!("Trajectories: {:?}",trajs);
            self.solver.train_an_epoch(trajs);
        }
    }

    pub fn save_policy(&self, file: String) {
        self.solver.save_policy(file);
    }
}
